...
<<echo=FALSE>>=
library("ggplot2")
source('mosaicplotdraft.R') #Alternate directory for Linux
source('proficiencypolygonfunction.R')
options(width=50)
source('simulate_data.R') #Load data by running R script to simulate 
#datasets
@
...
\frametitle{The Big Questions}
  ...
  \item Can we do analyses under constraints that remain rigorous and valid and provide accurate information?
  \end{itemize}
\end{frame}
...
\begin{center}
\textbf{What tools exist to help us turn data into usable information that informs decisions?} \\
\vspace{.2in}
\large \textbf{Policy Research}
\end{center}
\end{frame}
...
{
\usebackgroundtemplate{\includegraphics[height=.8\paperheight]{questiondice}}
\begin{frame}[plain]
